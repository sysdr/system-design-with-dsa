...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .metric-item .value.error { color: var(--error-text); font-size: 0.875rem; }
    .live-badge {
      display: inline-flex;
      align-items: center;
      gap: 0.35rem;
...
<body>
  ...
  <script>
    ...
    function fetchMetrics() {
      fetch('/api/metrics').then(function(r) { return r.json(); }).then(renderMetrics);
    }
    ...
    function setButtons(disabled) {
      ['op-full', 'op-user', 'op-problem', 'op-submission'].forEach(function(id) {
